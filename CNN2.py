...
class CNN_net:
    def __init__(self, is_training=True):
        ...
        if self.is_training:
            ...
            self.optimizer1 = tf.train.AdamOptimizer(learning_rate=self.learning_rate).minimize(self.total_loss)
    ...
    def build(self, inputs, is_training):
        ...
        with slim.arg_scope([slim.conv2d, slim.fully_connected],
            ...
            # drop1 = slim.dropout(full1, 0.6, is_training=is_training, scope='dropout1')
            # full2 = slim.fully_connected(drop1, 1024, scope='fc2')
            # drop2 = slim.dropout(full2, 0.6, is_training=is_training, scope='dropout2')
            # softmax = slim.fully_connected(drop2, self.class_size, activation_fn=None, scope='softmax')

            conv1 = slim.conv2d(inputs, 32, [3, 3], padding='SAME', scope='conv1')
            pool1 = slim.max_pool2d(conv1, 2, scope='pool1')
            conv2 = slim.conv2d(pool1, 64, [3, 3], padding='SAME')
            pool2 = slim.max_pool2d(conv2, 2, scope='pool2')
            conv3 = slim.conv2d(pool2, 128, [1, 1], padding='SAME')
            conv3 = slim.conv2d(conv3, 128, [3, 3], padding='SAME')
            conv3 = slim.conv2d(conv3, 128, [1, 1], padding='SAME')
            pool3 = slim.max_pool2d(conv3, 2, scope='pool3')
            conv4 = slim.conv2d(pool3, 512, [1, 1], padding='SAME')
            conv4 = slim.conv2d(conv4, 512, [3, 3], padding='SAME')
            conv4 = slim.conv2d(conv4, 512, [1, 1], padding='SAME')
            pool4 = slim.max_pool2d(conv4, 2, scope='pool3')

            trans = tf.transpose(pool4, [0, 3, 1, 2], name='trans_1')
            flat = slim.flatten(trans, scope='flat_1')
            full1 = slim.fully_connected(flat, 2048, scope='fc1')
